  ...
  for (curr = 0; curr < i; ++curr) {
    if (strcmp(room.room[curr].name, roomMsg->roomName) == 0) {
      ...
      for (curr2 = 0; curr2 < room.room[curr].sz; ++curr2) {
        write(room.room[curr].socks[curr2], roomMsg->msg,
              strlen(roomMsg->msg) + 1);
      }
    }
  }
}
...
int updateUserFile(sockaddr_in adresse) {
  FILE *fichier = NULL;
  fichier =
      fopen("users.txt", "r"); // en lecture pour rechercher l'adresse du client
  if (fichier != NULL) {
    int present = 0;
    char line[128];
    char ip[9];
...
int incrementInsult(sockaddr_in adresse) {
  ...
  if ((fichier != NULL) && (new != NULL)) {
    ...
    while (fgets(line, sizeof line, fichier) !=
           NULL) { // on copie tout le fichier user dans le nouveau en modifiant
                   // que la ligne de l'utilisateur voulu
      strncpy(ip, line, 8);
      ip[8] = '\0';
      sprintf(ipfile, "%d", adresse.sin_addr.s_addr);
      ipfile[8] = '\0';
      if (strcmp(ipfile, ip) == 0) { // c'est l'utilisateur vulgaire
        nbinsults = strtok(line, ":");
        nbinsults = strtok(NULL, ":");
        fprintf(new, "%d:%ld\n", adresse.sin_addr.s_addr,
                strtol(nbinsults, NULL, 10) + 1);
        if (atoi(nbinsults) >= 15) {
          // kick user
          int userCursor = 0;
          while (ips[userCursor].sin_addr.s_addr != adresse.sin_addr.s_addr &&
                 userCursor < 104) {
            userCursor++;
          }
...
void *renvoi_message(void *arg) {
  ...
  *sock = *s;
  ...
  while (1) {
    ...
    snprintf(date, sizeof date, "[%d:%d:%d]", instant.tm_hour, instant.tm_min,
             instant.tm_sec); // Date
    snprintf(message, sizeof message, "%c%s %s \n", command, date,
             buffer2); // Command + Date + Message
...
main(int argc, char **argv) {
  ...
  int enable = 1;
  if (setsockopt(socket_descriptor, SOL_SOCKET, SO_REUSEADDR, &enable,
                 sizeof(int)) < 0) {
  ...
  for (;;) {
    ...
    if (pthread_create(&t1, NULL, renvoi_message, &nouv_socket_descriptor) ==
        -1) {
      perror("pthread_create");
    }
  }
